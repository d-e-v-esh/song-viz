...
export const MeloDisc: React.FC<MeloDiscProps> = ({
  ...
  useEffect(() => {
    ...
    setCanvasContext(canvasRef.current!.getContext('2d') as CanvasRenderingContext2D)
  ...
  useEffect(() => {
    if (audioContext) {
      setAudioSource(audioContext.createMediaElementSource(audio as HTMLAudioElement))
  ...
  const drawSpectrum = () => {
    ...
    audioAnalyser.current!.getByteFrequencyData(dataArray.current as Uint8Array)
    ...
    if (bounceMultiplier) {
      ...
      const workingRMS = Math.max(baseRadiusValue, baseRadiusValue + currentRMS * bounceMultiplier)
      ...
      if (canvasContext) {
        ...
        if (centerImageSrc) {
          if (rotation) {
            ...
            canvasContext.drawImage(centerImage, -radius, -radius, radius * 2, radius * 2)
    ...
    for (var i = 0; i < bars; i++) {
      ...
      const xEnd = canvasRef.current!.width / 2 + Math.cos(radians * i) * (radius + barHeight)
      const yEnd = canvasRef.current!.height / 2 + Math.sin(radians * i) * (radius + barHeight)
