const Helper = () => {
  const __toRadians__ = (degree) => {
    return (degree * Math.PI) / 180;
  };

  const __rotatePoint__ = ([pointX, pointY], [originX, originY], degree) => {
    //clockwise
  ...
  const getPoints = (
    ...
    if (shape === "line") {
      ...
      for (let i = 0; i <= pointCount; i++) {
        ...
        let startingPoint = this.__rotatePoint__(
        ...
        let endingPoint = this.__rotatePoint__(
  ...
  const drawCircle = ([x, y], diameter, options = {}) => {
  ...
  const drawOval = ([x, y], height, width, options = {}) => {
  ...
  const drawSquare = ([x, y], diameter, options = {}) => {
    this.drawRectangle([x, y], diameter, diameter, options);
  };

  const drawRectangle = ([x, y], height, width, options = {}) => {
    let {
      color,
  ...
  const drawLine = ([fromX, fromY], [toX, toY], options = {}) => {
